...
public class Blacklist {
    ...
    public void add(String name) {
        if (useJson) {
            if (!jsonData.getData().getBlacklist().contains(name)) {
                jsonData.getData().getBlacklist().add(name);
                jsonData.saveData();
                jsonData.loadData();
            }
        } else {
            List<String> list = yamlData.getYaml().getStringList("blacklist");
            if (!list.contains(name)) {
                list.add(name);
                yamlData.getYaml().set("blacklist", list);
                yamlData.saveData();
                yamlData.reload();
            }
        }
    ...
    public void remove(String name) {
        if (useJson) {
            if (jsonData.getData().getBlacklist().contains(name)) {
                jsonData.getData().getBlacklist().remove(name);
                jsonData.saveData();
                jsonData.loadData();
            }
        } else {
    ...
    public boolean has(String name) {
        if (useJson) {
            return jsonData.getData().getBlacklist().contains(name);
        } else {
            return yamlData.getYaml().getStringList("blacklist").contains(name);
        }
    }
    ...
    public List<String> getList() {
        List<String> result;
        if (useJson) {
            result = new ArrayList<>(jsonData.getData().getBlacklist());
        } else {
            result = yamlData.getYaml().getStringList("blacklist");
        }
