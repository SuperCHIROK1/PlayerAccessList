...
public class WhitelistCommand implements CommandExecutor, TabCompleter {
    ...
    public WhitelistCommand(PlayerAccessList plugin) {
        ...
        this.list = plugin.getAccessLists().getWhitelist();
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!(sender.hasPermission("pal.admin")
                || sender.hasPermission("pal.whitelist"))) {
            Text.send(sender, msgs.getNoPerms());
            return true;
        }

        if (args.length == 0) {
            String msg = msgs.getUsage().replace("{command}", "/whitelist <toggle/list/add/remove>");
            Text.send(sender, msg);
            return true;
        }

        List<String> kickMessage = cfg.getWhitelistMessage();
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "toggle" -> {
                ...
                boolean enabled = cfg.toggleWl();

                String msg = msgs.getSwitchedTo()
                        .replace("{list}", msgs.getPhWhitelist())
            ...
            case "remove" -> {
                ...
                if (args.length == 2) {
                    ...
                    String msg = msgs.getUsage()
                            .replace("{command}", "/whitelist remove <ник>");
                    Text.send(sender, msg);
                }
                return true;
            ...
            case "add" -> {
                ...
                if (args.length == 2) {
                    ...
                    String msg = msgs.getUsage()
                            .replace("{command}", "/whitelist add <ник>");
                    Text.send(sender, msg);
                }
                return true;
